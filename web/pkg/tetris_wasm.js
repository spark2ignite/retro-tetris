 ...
 * @returns {boolean}
 */
export function get_game_active() {
    const ret = wasm.get_game_active();
    return ret !== 0;
}
 ...
 * @returns {number}
 */
export function get_spawn_count() {
    const ret = wasm.get_spawn_count();
    return ret >>> 0;
}
 ...
 */
export function queue_ai_move(rotations, target_x) {
    wasm.queue_ai_move(rotations, target_x);
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('tetris_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
